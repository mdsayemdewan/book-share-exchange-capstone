  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="w-full max-w-sm rounded-3xl bg-slate-800 border border-white/10 shadow-2xl p-8 flex flex-col gap-5">
        ...
        <h3 className="text-xl font-extrabold text-white text-center">Delete this share post?</h3>
        <p className="text-sm text-slate-400 text-center">
          <span className="font-semibold text-white">"{item.bookName}"</span> by{' '}
          <span className="text-indigo-300">{item.ownerName}</span> will be permanently deleted
          along with all related borrow requests. This cannot be undone.
        </p>
...
function StatusBadge({ status }) {
  const map = {
    ...
    borrowed: 'bg-blue-500/15 text-blue-300 border-blue-500/30',
...
export default function AdminShares() {
  ...
  const confirmDelete = async () => {
    ...
    setError('');
    try {
      await api(`/api/admin/shares/${item.id}`, { method: 'DELETE' });
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-200">
      ...
      <div className="relative border-b border-white/10 bg-slate-800/50 backdrop-blur-xl overflow-hidden">
        <div className="absolute top-0 right-0 -translate-y-1/2 translate-x-1/4 w-96 h-96 bg-emerald-500/15 rounded-full blur-3xl mix-blend-screen opacity-50 pointer-events-none" />
        <div className="mx-auto max-w-5xl px-4 py-10 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-5">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-bold uppercase tracking-widest mb-4">
                 Admin · Share Posts
              </div>
              <h1 className="text-4xl font-extrabold text-white tracking-tight drop-shadow-sm">Manage Share Posts</h1>
              <p className="mt-2 text-sm text-slate-400 font-medium max-w-lg">
                View and delete any user's book share listing from the platform.
              </p>
            </div>
            <button
              onClick={load}
        ...
        <div className="rounded-3xl border border-white/10 bg-white/5 shadow-2xl backdrop-blur-md overflow-hidden">
          <div className="px-8 py-5 border-b border-white/10 bg-white/[0.02]">
            <h3 className="font-bold text-white flex items-center gap-3 text-lg">
              All Share Posts
              <span className="inline-flex items-center justify-center rounded-xl bg-emerald-500/30 border border-emerald-500/40 text-emerald-200 px-3 py-1 text-xs font-extrabold">
                {books.length}
              </span>
            </h3>
          </div>
          ...
          {loading ? (
            ...
            <div className="p-20 text-center flex flex-col items-center">
              ...
              <h3 className="text-2xl font-extrabold text-white mb-2">No share posts yet</h3>
              <p className="text-slate-400 text-sm">No book share listings have been created on the platform.</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="h-16 w-16 sm:h-20 sm:w-20 shrink-0 rounded-2xl overflow-hidden bg-slate-800 border border-white/10 shadow-md">
                      {book.imageUrls?.[0] ? (
                        <img src={book.imageUrls[0]} alt={book.bookName} className="h-full w-full object-cover" />
                      ) : (
                    ...
                    <div className="min-w-0">
                      <div className="font-extrabold text-white text-base truncate">{book.bookName}</div>
                      <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
                        <span className="text-xs text-slate-400"><span className="opacity-60">By</span> {book.ownerName}</span>
                        <span className="text-xs text-slate-500 truncate">{book.ownerEmail}</span>
                      </div>
                      <div className="flex flex-wrap gap-2 mt-2 items-center">
                        <StatusBadge status={book.status} />
                        <span className="text-xs bg-white/5 border border-white/10 text-slate-400 px-2.5 py-0.5 rounded-full">{book.category}</span>
                        <span className="text-xs bg-white/5 border border-white/10 text-slate-400 px-2.5 py-0.5 rounded-full">{book.condition}</span>
                        {book.borrowDuration && (
                          <span className="text-xs bg-white/5 border border-white/10 text-slate-400 px-2.5 py-0.5 rounded-full">{book.borrowDuration}d</span>
                        )}
                      </div>
